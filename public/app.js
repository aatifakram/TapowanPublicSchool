...
(function () {
  ...
  async function seedDemoDataIfEmpty() {
    ...
    const seedInvest = !store[INVEST_KEY] || store[INVEST_KEY].length === 0;
    const seedIncome = !store[INCOME_KEY] || store[INCOME_KEY].length === 0;
    const seedExpense = !store[EXPENSE_KEY] || store[EXPENSE_KEY].length === 0;
  ...
  window.renderAll = function () {
    ...
    setTimeout(() => { injectDashboardCard(); injectFinanceNavItem(); }, 80);
  };

  /* ─── INIT ───────────────────────────────────────────────── */
  async function financeInit() {
    // Wait until serverStore is populated (boot() runs first)
    const check = () => {
      if (typeof serverStore !== "undefined" && serverStore !== null) {
