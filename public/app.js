...
(function () {
  ...
  /* ─── SEED DEMO DATA (only if server has no data yet) ─────── */
  async function seedDemoDataIfEmpty() {
    const store = serverStore || {};

    const seedInvest = !Array.isArray(store[INVEST_KEY]) || store[INVEST_KEY].length === 0;
    const seedIncome = !Array.isArray(store[INCOME_KEY]) || store[INCOME_KEY].length === 0;
    const seedExpense = !Array.isArray(store[EXPENSE_KEY]) || store[EXPENSE_KEY].length === 0;
    ...
    // Insert seeds via API so they persist on the server
    try {
      await Promise.all([
        ...investSeeds.map(r => api(`/api/modules/${INVEST_KEY}`, { method: "POST", body: JSON.stringify(r) })),
        ...incomeSeeds.map(r => api(`/api/modules/${INCOME_KEY}`, { method: "POST", body: JSON.stringify(r) })),
        ...expenseSeeds.map(r => api(`/api/modules/${EXPENSE_KEY}`, { method: "POST", body: JSON.stringify(r) })),
      ]);
      await loadStore();
    } catch (e) {
      console.warn("Finance seed failed:", e.message);
    }
  }
  ...
  function calcFinancials(dateFrom, dateTo) {
    ...
    const hasFeeIncomeEntries = (store[INCOME_KEY] || []).some(r => r.category === "Fees");
  ...
  function getDayWise(dateFrom, dateTo) {
    ...
    // Only add fees module data if schoolIncome has no "Fees" category entries
    const hasFeeIncomeEntries = (store[INCOME_KEY] || []).some(r => r.category === "Fees");
  ...
  let financeState = {
    view: "overview",
    dateFrom: new Date(new Date().getFullYear(), 3, 1).toISOString().slice(0, 10),
    dateTo: finTodayStr(),
    dayFilter: "thisMonth",
    investTab: "active",
  };
  ...
  function balanceBar(income, expense, invested) {
    ...
    return `
      <div style="display:flex;gap:8px;align-items:center;margin-bottom:10px;"><div style="flex:1;background:rgba(255,255,255,0.08);border-radius:8px;height:18px;overflow:hidden;display:flex;"><div style="background:#4ade80;width:${ip}%;"></div></div><span style="color:#4ade80;font-size:0.72rem;width:60px;text-align:right;">${fmtK(income)}</span></div>
  ...
  function applyQuickFilter(key) {
    ...
    else if (key === "thisQuarter") { const qm = Math.floor(m/3)*3; financeState.dateFrom = `${y}-${String(qm+1).padStart(2,"0")}-01`; financeState.dateTo = finTodayStr(); }
    else if (key === "thisYear") { financeState.dateFrom = `${y}-04-01`; financeState.dateTo = finTodayStr(); }
  ...
  function bindContentEvents(content) {
    ...
    content.querySelector("#fi_exportInc")?.addEventListener("click", () => {
      const list = (getFinStore()[INCOME_KEY] || []).filter(r => r.date >= financeState.dateFrom && r.date <= financeState.dateTo);
    ...
    content.querySelector("#fi_exportExp")?.addEventListener("click", () => {
      const list = (getFinStore()[EXPENSE_KEY] || []).filter(r => r.date >= financeState.dateFrom && r.date <= financeState.dateTo);
